...
def on_message(client, userdata, msg):
    try:
        ...
        if not keypoints.pose_landmarks:
            filename = f"{prefix}_{unique_id}_no_landmarks.jpg"
            save_path = os.path.join(output_folder, filename)
            cv2.putText(image, "No landmarks detected", (30, 50), font, 1, colors["red"], 2)
            cv2.imwrite(save_path, image)
            cursor.execute(
                "INSERT INTO posture_log (pi_id, filename, received_time, analyzed_time, neck_angle, body_angle, posture_status, landmarks_detected) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (prefix, filename, received_time, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), None, None, "Unknown", False)
            )
            conn.commit()
            print(f"No landmarks found. Saved raw image to {save_path}")
            return
        ...
        if not is_valid:
            filename = f"{prefix}_{unique_id}_partial_landmarks.jpg"
            save_path = os.path.join(output_folder, filename)
            ...
            conn.commit()
            print(f"Incomplete landmarks. Saved image to {save_path}")
            return
        ...
        cursor.execute(
            "INSERT INTO posture_log (pi_id, filename, received_time, analyzed_time, neck_angle, body_angle, posture_status, landmarks_detected) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (prefix, filename, received_time, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), neck_angle, body_angle, "Good" if posture_ok else "Bad", True)
        )
        conn.commit()

        print(f"Analyzed and saved to {save_path}")
